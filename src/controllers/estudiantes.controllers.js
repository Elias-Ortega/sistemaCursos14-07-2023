...
const update = async (req = request, res = response) => {
  try {
    ...
    if (estudiante) {
      ...
      res.status(400).json({
        mensajePersonalizado: 'Error en el cliente, update estudiante, no existe este estudiante',
        estudiante: req.body
      });
    }
...
const deleteById = async (req = request, res = response) => {
  try {
    ...
    res.status(500).json({
      ...
      code: 500,
      name: error.name,
      mensajePersonalizadoDesarrollador: 'Error en el servidor, deleteById  estudiante'
